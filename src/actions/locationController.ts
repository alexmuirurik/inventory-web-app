...
export const getLocationById = async (locationId: string) => {
    try {
        const location = await prisma.businessLocation.findFirst({where: { id: locationId}})
        return location
    } catch (error) {
...
export const setActiveLocation = async (userId: string, businessId: string, locationId: string, name: string) => {
    try {
        ...
        console.log('We have faced an error setting location as active ' + error)
    }
}

export const createLocation = async (userId: string, data: z.infer<typeof businessSchema>, businessId: string) => {
    try {
        const location = await getLocation(businessId, data.location)
        if(location) return await setActiveLocation(userId, businessId, location.id, location.name)
        ...
        const setactivelocation = await setActiveLocation(userId, businessId, createdlocation.id, createdlocation.name)
